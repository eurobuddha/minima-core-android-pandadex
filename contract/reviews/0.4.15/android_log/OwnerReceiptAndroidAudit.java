...
public final class OwnerReceiptAndroidAudit extends Instrumentation {
    ...
    void crash()throws Exception {
        ...
        check(prefs("owner_audit_checkpoint").edit().putInt("pid",android.os.Process.myPid()).putInt("checks",checks)
                .putInt("build",48).putBoolean("ready",true).commit(),"crash checkpoint committed");
    ...
    void feedbackAndKeyboard(MainActivity activity)throws Exception {
        ...
        mainChecked(()->{
            appView(activity,"logBox").performClick();
            try {
                androidx.appcompat.app.AlertDialog dialog=(androidx.appcompat.app.AlertDialog)field(activity,MainActivity.class,"activityLogDialog");
                check(dialog.isShowing(),"expanded log opens");
                activity.setStage("Live modal: signing");
                android.widget.TextView text=(android.widget.TextView)field(activity,MainActivity.class,"logDialogText");
                check(text.getText().toString().contains("Live modal: signing"),"new event appears without reopening modal");
                activity.setStage("Live modal: submitted, awaiting confirmation");
                check(dialog==field(activity,MainActivity.class,"activityLogDialog")&&dialog.isShowing(),"same open modal receives successive events");
                check(text.getText().toString().split("\\n\\n")[0].endsWith("Live modal: submitted, awaiting confirmation"),"latest event remains first");
                dialog.dismiss();
            } catch(Exception failure){throw new IllegalStateException(failure);}
        });
        waitForIdleSync();
        mainChecked(()->{
            try {
                check(field(activity,MainActivity.class,"logDialogText")==null&&field(activity,MainActivity.class,"activityLogDialog")==null,"dismissal releases dialog references");
                activity.setStage("Live modal: confirmed");appView(activity,"logBox").performClick();
                check(((android.widget.TextView)field(activity,MainActivity.class,"logDialogText")).getText().toString().contains("Live modal: confirmed"),"reopened log includes events received while closed");
                ((androidx.appcompat.app.AlertDialog)field(activity,MainActivity.class,"activityLogDialog")).dismiss();
            } catch(Exception failure){throw new IllegalStateException(failure);}
        });
    ...
    void offlineActivity()throws Exception {
        ...
        try {
            ...
            mainChecked(()->{
                ...
                check(hasText(appView(second,"logBox"),"Live modal: confirmed"),"timestamped progress survives Activity recreation");
