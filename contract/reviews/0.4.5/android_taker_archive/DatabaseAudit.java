...
public final class DatabaseAudit extends Instrumentation {
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            else { correctionAndRollback(); takerScope(); migration(); reinclusion(); legacyReinclusion(); boundedRequeue(); takerCompletion(); takerArchive(); }
    ...
    private void fresh() { if(db!=null)db.close();getTargetContext().deleteDatabase("pandadex.db");db=new DexDb(getTargetContext());check(db.getWritableDatabase().getVersion()==10,"fresh schema10"); }
    ...
    private void migration() throws Exception {
        for(int version:new int[]{4,6,7,8,9}) {
            ...
            check(db.getWritableDatabase().getVersion()==10,"SQLiteOpenHelper migration"+version+"to10");
        ...
        passed.append("actual SQLiteOpenHelper schema4/6/7/8/9to10 upgrades\n");
    }
