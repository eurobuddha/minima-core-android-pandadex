...
public final class ExportLifecycleAudit extends Instrumentation {
    ...
    public static final class Host extends ComponentActivity {
        ...
        @Override public void onDestroy(){super.onDestroy();destroyed.countDown();}
    ...
    void close()throws Exception{Host h=host;ui(h::finish);if(!h.destroyed.await(8,TimeUnit.SECONDS))throw new AssertionError("Activity did not finish");waitForIdleSync();}
    ...
    void normal()throws Exception {
        ...
        ui(()->{
            model.start(new TradeExport.Snapshot());
            second[0]=new TradeExportSession(host.getApplication(),memory,new FakeWork());
        });close();TradeExportFiles.Prepared abandoned=prepared();ui(()->work.build.onDone(abandoned));check(!abandoned.zip.exists(),"late build after finish cleaned up");
        // An existing idle owner must not overwrite a just-finished owner's journal.
        ui(()->{
            second[0].start(new TradeExport.Snapshot());
            check(second[0].phase()==TradeExportSession.Phase.NOTICE,"existing second screen reloads terminal journal before starting");
            second[0].onCleared();
        });
