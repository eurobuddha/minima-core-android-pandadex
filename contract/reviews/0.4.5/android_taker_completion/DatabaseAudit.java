...
public final class DatabaseAudit extends Instrumentation {
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            else { correctionAndRollback(); takerScope(); migration(); reinclusion(); legacyReinclusion(); boundedRequeue(); takerCompletion(); }
    ...
    private void crashTaker(boolean committed) throws Exception {
        ...
        check(db.recordTakerFill(Collections.singletonList(o.coinid),spend(o,"0xaaaa",100),new BigDecimal("0.01"),new BigDecimal("100"),true,"BOOK"),"actual taker writer ran before crash");
    ...
    private void recoverTaker(boolean committed) throws Exception {
        ...
        db=new DexDb(getTargetContext());check(count("mytrade")== (committed?1:0),"only committed taker row survives");
        Order5 o=order();boolean inserted=db.recordTakerFill(Collections.singletonList(o.coinid),spend(o,"0xaaaa",100),new BigDecimal("0.01"),new BigDecimal("100"),true,"BOOK");
        check(inserted!=committed&&count("mytrade")==1,"retry restores or reuses exactly one taker row");
        check(prefs.edit().remove("pending").commit(),"pending cleanup acknowledged after durable row");
        db.close();db=new DexDb(getTargetContext());
        check(count("mytrade")==1&&prefs.getString("pending","").isEmpty(),"row retained after acknowledged cleanup and reopen");
