...
repo=Path('/Users/eurobuddha/Projects/minima/apks/pandadex');out=Path('/private/tmp/pandadex-android-audit-build-28')
root=Path(Path('/private/tmp/pandadex-review-emulator-28-location').read_text().strip());assert str(root).startswith('/private/tmp/pandadex-review-emulator-')
ps=subprocess.check_output(['ps','-axo','pid=,command='],text=True);assert any('qemu' in line and '-avd PandaDexAudit28 ' in line and str(root/'avd') in line for line in ps.splitlines())
...
system.update(adb_server_port=5049,emulator_name='PandaDexAudit28',user_device_accessed=False,user_node_accessed=False)
(out/'android-system.json').write_text(json.dumps(system,indent=2)+'\n')
print(run('install',str(out/'pandadex-export-audit-28.apk')),flush=True)
...
for phase in ['normal','crash','recover']:
 ...
 if phase=='crash':
  ...
  assert values.get('ready')=='true' and values.get('build')=='28',values
