...
shutil.copy2(here/'PriceClockAndroidAudit.java',runner/'PriceClockAndroidAudit.java')
...
<application android:label="PandaDEX Clock Audit" android:allowBackup="false" />
<instrumentation android:name="com.eurobuddha.pandadex.PriceClockAndroidAudit" android:targetPackage="com.eurobuddha.pandadex.audit" android:functionalTest="true" />
</manifest>''')
def hashes():
...
(out/'build-evidence.json').write_text(json.dumps({
    'purpose':'Actual Android elapsed-clock and scheduled heartbeat type; no node or stock-device approval',
    'application_id':'com.eurobuddha.pandadex.audit','audit_build':args.build_number,
    'apk_sha256':hashlib.sha256(apk.read_bytes()).hexdigest(),
    'runner_sha256':hashlib.sha256((runner/'PriceClockAndroidAudit.java').read_bytes()).hexdigest(),
