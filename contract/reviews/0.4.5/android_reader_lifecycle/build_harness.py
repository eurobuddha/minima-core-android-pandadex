#!/usr/bin/env python3
"""Build a separate Android SDK transport harness; never builds the production package.
...
runner=project/'app/src/main/java/org/minimarex/minimaapi';runner.mkdir(parents=True)
shutil.copy2(here/'SDKAndroidAudit.java',runner/'SDKAndroidAudit.java')
bridge=project/'app/src/main/java/com/eurobuddha/pandadex';bridge.mkdir(parents=True)
shutil.copy2(here/'BridgeLifecycleAudit.java',bridge/'BridgeLifecycleAudit.java')
...
source=re.sub(r'versionName "[^"]+"',f'versionName "sdk-audit-{args.build_number}"',source,count=1)
...
source=source[:start]+'''    sourceSets {
        main {
            ...
            res.srcDirs = ['src/main/res', '''+repr(str(repo/'app/src/main/res'))+''']
        }
    }
'''+source[end:]
...
<application android:label="PandaDEX SDK Audit" android:allowBackup="false">
<provider android:name="androidx.core.content.FileProvider" android:authorities="com.eurobuddha.pandadex.audit.replies" android:exported="false" android:grantUriPermissions="true">
<meta-data android:name="android.support.FILE_PROVIDER_PATHS" android:resource="@xml/sdk_audit_paths" />
</provider>
<provider android:name="org.minimarex.minimaapi.SDKAndroidAudit$BlockingProvider" android:authorities="com.eurobuddha.pandadex.audit.blocked" android:exported="false" />
</application>
<instrumentation android:name="org.minimarex.minimaapi.SDKAndroidAudit" android:targetPackage="com.eurobuddha.pandadex.audit" android:functionalTest="true" />
</manifest>''')
resources=project/'app/src/main/res/xml';resources.mkdir(parents=True)
(resources/'sdk_audit_paths.xml').write_text('<paths><cache-path name="responses" path="sdk-audit/" /></paths>')
def hashes():
...
apk=out/f'pandadex-sdk-audit-{args.build_number}.apk'
shutil.copy2(project/'app/build/outputs/apk/debug/app-debug.apk',apk)
(out/'build-evidence.json').write_text(json.dumps({
    'purpose':'Isolated Android SDK tests with all outgoing broadcasts intercepted; not a release or stock-node approval',
    'application_id':'com.eurobuddha.pandadex.audit','audit_build':args.build_number,
    'apk_sha256':hashlib.sha256(apk.read_bytes()).hexdigest(),
    'runner_sha256':hashlib.sha256((runner/'SDKAndroidAudit.java').read_bytes()).hexdigest(),
    'bridge_runner_sha256':hashlib.sha256((bridge/'BridgeLifecycleAudit.java').read_bytes()).hexdigest(),
