...
public final class OwnerReceiptAndroidAudit extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        if("normal".equals(phase)){normal();keyLoading();upkeep();integrity();schema();ownerHistory();ownerAccess();archiveIntegrity();ownerRevisions();ownerQueue();makerIdle();makerSnapshots();makerAuthorization();}else if("crash".equals(phase))crash();else if("recover".equals(phase)){recover();ownerRecovery();ownerQueueAfterDeath();}else throw new AssertionError("unknown phase");
    ...
    void crash()throws Exception {
        ...
        check(prefs("owner_audit_checkpoint").edit().putInt("pid",android.os.Process.myPid()).putInt("checks",checks)
                .putInt("build",30).putBoolean("ready",true).commit(),"crash checkpoint committed");
    ...
    void recover()throws Exception {
        ...
        check(checkpoint.getBoolean("ready",false)&&checkpoint.getInt("build",0)==30,"prior crash checkpoint belongs to audit30");
    ...
    void archiveIntegrity()throws Exception {
        JSONObject expected=new JSONObject().put("intent","audit30_sweep").put("sources",new JSONArray().put("0xfaaa").put("0xfbbb"))
