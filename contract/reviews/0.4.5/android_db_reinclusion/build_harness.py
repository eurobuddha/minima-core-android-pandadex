...
source=re.sub(r'versionName "[^"]+"',f'versionName "database-audit-{args.build_number}"',source,count=1)
...
<application android:label="PandaDEX Database Audit" android:allowBackup="false" />
...
apk=out/f'pandadex-database-audit-{args.build_number}.apk'
shutil.copy2(project/'app/build/outputs/apk/debug/app-debug.apk',apk)
(out/'build-evidence.json').write_text(json.dumps({
    'purpose':'Isolated Android SQLite wrapper tests; not a PandaDEX release or stock-device approval',
