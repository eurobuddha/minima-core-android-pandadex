...
root=Path(Path('/private/tmp/pandadex-review-emulator-19-location').read_text().strip())
...
def pids():
 ...
 return [int(x.strip().split(None,1)[0]) for x in lines if 'qemu' in x and '-avd PandaDexAudit19 ' in x and str(root/'avd') in x]
...
Path('/private/tmp/pandadex-android-audit-build-19/cleanup.json').write_text(json.dumps({'emulator_stopped':True,'isolated_adb_server_stopped':True,'temporary_userdata_and_keys_removed':True,'audit_apks_retained':True,'production_apks_unchanged':True},indent=2)+'\n')
print('Stopped only PandaDexAudit19 and isolated ADB5049; removed its userdata/keys; retained APK and logs')
