...
public final class OwnerReceiptAndroidAudit extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        if("normal".equals(phase)){normal();keyLoading();upkeep();integrity();schema();ownerHistory();ownerAccess();archiveIntegrity();ownerRevisions();ownerQueue();makerIdle();makerSnapshots();makerAuthorization();}else if("crash".equals(phase))crash();else if("recover".equals(phase)){recover();ownerRecovery();ownerQueueAfterDeath();}else if("app".equals(phase))offlineActivity();else throw new AssertionError("unknown phase");
    ...
    void crash()throws Exception {
        ...
        check(prefs("owner_audit_checkpoint").edit().putInt("pid",android.os.Process.myPid()).putInt("checks",checks)
                .putInt("build",31).putBoolean("ready",true).commit(),"crash checkpoint committed");
    ...
    void recover()throws Exception {
        ...
        check(checkpoint.getBoolean("ready",false)&&checkpoint.getInt("build",0)==31,"prior crash checkpoint belongs to audit31");
    ...
    void archiveIntegrity()throws Exception {
        JSONObject expected=new JSONObject().put("intent","audit31_sweep").put("sources",new JSONArray().put("0xfaaa").put("0xfbbb"))
    ...
    void offlineActivity()throws Exception {
        ...
        try {
            ...
            for(int i=0;i<labels.length;i++) {
                ...
                mainChecked(()->{
                    ...
                    try {java.lang.reflect.Field f=MainActivity.class.getDeclaredField(field);f.setAccessible(true);check(((android.view.View)f.get(first)).isShown(),label+" tab is displayed");}
                    catch(ReflectiveOperationException failure){throw new IllegalStateException(failure);}
        ...
        }finally{final MainActivity current=activity;mainChecked(current::finish);waitForIdleSync();}
