...
repo=Path('/Users/eurobuddha/Projects/minima/apks/pandadex');out=Path('/private/tmp/pandadex-android-audit-build-47')
root=Path(Path('/private/tmp/pandadex-review-emulator-45-location').read_text().strip());assert str(root).startswith('/private/tmp/pandadex-review-emulator-')
ps=subprocess.check_output(['ps','-axo','pid=,command='],text=True);assert any('qemu' in line and '-avd PandaDexAudit45 ' in line and str(root/'avd') in line for line in ps.splitlines())
...
system.update(adb_server_port=5049,emulator_name='PandaDexAudit45',user_device_accessed=False,user_node_accessed=False)
...
print(run('install','-r',str(out/'pandadex-export-audit-47.apk')),flush=True)
...
for phase in ['normal','crash','recover']:
 ...
 if phase=='crash':
  ...
  assert values.get('ready')=='true' and values.get('build')=='47',values
