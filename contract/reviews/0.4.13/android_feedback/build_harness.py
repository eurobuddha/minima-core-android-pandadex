...
for audit_source in here.glob('*.java'): shutil.copy2(audit_source,runner/audit_source.name)
...
<application android:label="PandaDEX Owner Access Audit" android:allowBackup="false" android:theme="@android:style/Theme.Material.NoActionBar"><activity android:name="com.eurobuddha.pandadex.OwnerHistoryAuditActivity" android:exported="false" />
<activity android:name="com.eurobuddha.pandadex.MainActivity" android:exported="false" android:windowSoftInputMode="adjustResize" android:configChanges="keyboard|keyboardHidden|orientation|screenSize|screenLayout|smallestScreenSize|uiMode|navigation" android:theme="@style/Theme.PandaDex" />
<service android:name="com.eurobuddha.pandadex.NodeTransportService" android:exported="false" android:process=":nodeipc" /></application>
<instrumentation android:name="com.eurobuddha.pandadex.OwnerReceiptAndroidAudit" android:targetPackage="com.eurobuddha.pandadex.audit" android:functionalTest="true" />
</manifest>''')
def hashes():
...
assert 'com.eurobuddha.pandadex.MainActivity' in manifest
assert 'NodeTransportService' in manifest
assert 'DexKeepAliveService' not in manifest
...
(out/'build-evidence.json').write_text(json.dumps({
    'purpose':'Real Android component regressions plus full offline MainActivity tabs/recreation; no node or stock-device approval',
    'application_id':'com.eurobuddha.pandadex.audit','audit_build':args.build_number,
    'apk_sha256':hashlib.sha256(apk.read_bytes()).hexdigest(),
    'runner_sha256':hashlib.sha256((runner/'OwnerReceiptAndroidAudit.java').read_bytes()).hexdigest(),
    'audit_java_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in runner.glob('*.java')},
