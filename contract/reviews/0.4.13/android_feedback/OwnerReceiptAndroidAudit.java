...
public final class OwnerReceiptAndroidAudit extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        if("normal".equals(phase)){normal();keyLoading();upkeep();integrity();schema();ownerHistory();ownerAccess();archiveIntegrity();ownerRevisions();ownerQueue();makerIdle();makerSnapshots();makerAuthorization();identityQueue();watcherIdentity();watcherBlock();poolMarket();transactionProgress();}else if("crash".equals(phase))crash();else if("recover".equals(phase)){recover();ownerRecovery();ownerQueueAfterDeath();}else if("app".equals(phase))offlineActivity();else throw new AssertionError("unknown phase");
    ...
    void crash()throws Exception {
        ...
        check(prefs("owner_audit_checkpoint").edit().putInt("pid",android.os.Process.myPid()).putInt("checks",checks)
                .putInt("build",43).putBoolean("ready",true).commit(),"crash checkpoint committed");
    ...
    void recover()throws Exception {
        ...
        check(checkpoint.getBoolean("ready",false)&&checkpoint.getInt("build",0)==43,"prior crash checkpoint belongs to audit43");
    ...
    void feedbackAndKeyboard(MainActivity activity)throws Exception {
        ...
        for(boolean landscape:new boolean[]{false,true}) {
            ...
            mainChecked(()->{
                amount.setText("123.45");amount.requestFocus();amount.requestRectangleOnScreen(new android.graphics.Rect(0,0,amount.getWidth(),amount.getHeight()),true);
            ...
            mainChecked(()->button.requestRectangleOnScreen(new android.graphics.Rect(0,0,button.getWidth(),button.getHeight()),true));
            SystemClock.sleep(300);waitForIdleSync();
            screenCapture("app-keyboard-"+(landscape?"landscape":"portrait")+".png");
            mainChecked(()->{aboveKeyboard(activity,button);aboveKeyboard(activity,appView(activity,"logBox"));check((amount.getImeOptions()&android.view.inputmethod.EditorInfo.IME_FLAG_NO_EXTRACT_UI)!=0,"landscape editor cannot replace the trade page");});
        ...
        screenCapture("app-progress-log.png");
    }
