...
public class NodeApi {
    ...
    public static final String ERR_WRITE_UNCERTAIN = "The node has not confirmed the outcome of a previous write. "
            + "It may still complete. New signing is paused. Check Orders, Tape and Assets, then use Assets → Resolve interrupted write.";
    ...
    public void reRegister() {
        ...
        cmd("__register__", new Cb() {
            public void onResult(JSONObject reply) { noteEnabled(reply.optBoolean("enabled", false)); }
            public void onError(String message) { noteEnabled(false); }
        });
    }
    ...
    public String interruptedWriteDetails() {
        return writePrefs().getString("command", "Command details unavailable for this older receipt.");
    }
        ...
        IPC_QUEUE.submit(finish -> dispatch(command, new Cb() {
            ...
            private void complete(JSONObject reply, String error) {
                ...
                try {
                    ...
                    boolean funds = writesFunds(command);
    ...
    private void dispatch(String command, Cb cb, java.util.function.BooleanSupplier authorized) {
        ...
        android.util.Log.d("PandaDexIPC", writeId + " begin " + verb);
        if (funds) {
            ...
            if (!writePrefs().edit().putString("pending", writeId).putString("command", command)
                    .putLong("started", System.currentTimeMillis()).commit()) {
        ...
        final Runnable timeout = () -> {
            ...
            android.util.Log.w("PandaDexIPC", writeId + " timeout " + verb);
            if (funds && writeId.equals(activeWrite)) activeWrite = "";
            if (++consecutiveTimeouts >= 3) noteEnabled(false);
        ...
        try { mApi.Command(command, new MinimaAPIListener() {
            ...
            public void response(JSONObject zResponse) {
                mMain.post(() -> {
                    android.util.Log.d("PandaDexIPC", writeId + " reply " + verb + " status="
                            + (zResponse != null && zResponse.optBoolean("status", false))
                            + " chars=" + (zResponse == null ? 0 : zResponse.toString().length()));
                    // Save immutable transaction identity even for late replies, before callbacks.
                    if (funds && zResponse != null && zResponse.optBoolean("status", false)
                            && !zResponse.optBoolean("pending", false))
                        SubmissionIds.remember(mContext.getApplicationContext(), zResponse, command);
                    ...
                    if (funds && writeId.equals(activeWrite)) activeWrite = "";
                    if (funds && isCompleteReply(zResponse) && !zResponse.optBoolean("pending", false) && !isTooLong(zResponse) && writeId.equals(writePrefs().getString("pending", "")))
                    ...
                    try {
                        ...
                        if (funds && zResponse.optBoolean("pending", false)) {
                            cb.onError(ERR_WRITE_UNCERTAIN); return;
                        }
