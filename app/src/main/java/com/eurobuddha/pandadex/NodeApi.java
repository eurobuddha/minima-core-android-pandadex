 ...
 *    (this app targets the UPSTREAM node, which has no file-response fallback).
...
public class NodeApi {
    ...
    private void noteEnabled(boolean enabled) {
        ...
        mEnabled = enabled;
        android.util.Log.d("PandaDex", "pairing -> " + enabled);
    ...
    public void reRegister() {
        ...
        if (mPendingWrites > 0) {   // a slow write (PoW) is in flight — recreating would drop its reply
            android.util.Log.d("PandaDex", "reREGISTER skipped — write in flight");
            return;
        }
        android.util.Log.d("PandaDex", "reREGISTER");
